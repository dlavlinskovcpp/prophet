...
The active release surface includes `initialize_market_v2`, `initialize_notary_config`,
...
`resolve_market_threshold`, `redeem`,
...
discriminators, and account allocation sizes are frozen by the generated IDL
under `target/idl/` after a pinned build. The linked migration snapshot remains
historical evidence, not the active ABI definition.
...
Frozen PDA seeds are market `(b"market", creator, resolver_hash, open_ts_le,
market_nonce_le)`, notary config `(b"notary_config", authority)`, order
`(b"order", market, owner, seq_le)`, and position `(b"position", market,
owner)`. The authoritative active derivations are in
`sdk/python/prophet_sdk/pdas.py`.
...
`resolve_market_threshold` is the sole instruction that may resolve a market,
including with `Invalid`. No market authority instruction can write settlement
outcome, proof hash, public-input hash, or resolution timestamp.
