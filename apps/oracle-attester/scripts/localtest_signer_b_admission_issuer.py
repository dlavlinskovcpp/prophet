"""Localtest-only fixed issuer-B initializer and one-shot G1 issuer."""
...
def initialize_localtest_signer_b_issuer(state_dir: str, *, issuer_id: str, key_id: str) -> dict[str, object]:
    key = _key(_seed_path(state_dir))
    return {"schema": "PROPHET_LOCALTEST_ADMISSION_ISSUER_V1", "version": 1, "role": "B", "issuer_id": issuer_id, "key_id": key_id, "public_key": str(key.pubkey()), "ready": True}
...
def main() -> None:
    ...
    metadata = initialize_localtest_signer_b_issuer(args.state_dir, issuer_id=args.issuer_id, key_id=args.key_id)
